...
# Hanging Man pattern exists when the body of the candlestick is atmost half of the lower shadow of the candlestick.
...
df['PaperUmbrella']=np.where( ( (abs(df["Close"]-df["Open"]))/df["Open"] <=0.015) &
...
rslt_df = df[df['PaperUmbrella']==1]



# Hanging Man is useful only when it's preceded by a bullish pattern. If no such pattern exists, a Hanging Man pattern isn't formed.
...
for i in bullish_pattern_exists:
    if(i=="Yes"):
        print("Hanging Man Pattern formed")
    else:
